...
population = []
for i in xrange(4):
    population.append([generateRandomStrategy() for i in xrange(POP_SIZE)])


def runGen(threadNum, strategy, steps=STEPS, init=CANS):
    rw[threadNum].graphicsOff()
    ...
    if type(init) is str:
        ...
        rw[threadNum].load(init)
    elif type(init) in [int, float] and 0 <= init <= 1:
        # init is a can density
        rw[threadNum].goto(0, 0)
        rw[threadNum].distributeCans(init)
    else:
    ...
    for i in xrange(steps):
        p = rw[threadNum].getPerceptCode()
        action = robby.POSSIBLE_ACTIONS[int(strategy[p])]
        reward += rw[threadNum].performAction(action)
    return reward


def simulateGeneration(mutationRate, threadNum):

    output_file = 'GAoutput_{0}.txt'.format(mutationRate)
    print("writing to file: " + output_file)

    # loop over #generations 
    for g in xrange(GENERATIONS): 
        results = []
    # generate set of random maps and save them
    #for i in range(5):
    #    rw.distributeCans(CANS)
    #    rw.goto(0,0)
    #    rw.save('config_{0}.txt'.format(i))

    # loop over population, testing each map
    #for strategy in population: 
    #    total_score = sum([runGen(strategy, steps=STEPS, init='config_{0}.txt'.format(i)) for i in range(5)])
    #    results.append((total_score / 5, strategy))

        for strategy in population[threadNum]:
            results.append((runGen(threadNum, strategy, steps=STEPS, init=CANS), strategy))
        # sort the results
        ...
        population[threadNum] = list(map(lambda x: x[1], results))
        ...
        # every 50 generations, demo the best performer
        #    rw.demo(bestStrategy, steps=STEPS, init='config_0.txt')

        # select next generation / mutate / crossover -- Jack
        # take the top 4 from previous generation

        indicies = []
        newPopulation = population[threadNum][:10]
        for p in range((len(population[threadNum]) - len(newPopulation)) // 2):
            # get parents
            ...
            raw_indicies = np.array(list(reversed(range(len(population[threadNum])))), dtype='float') ** 2

            p = raw_indicies / np.sum(raw_indicies) 
            parent1, parent2 = np.random.choice(population[threadNum], size=2, replace=False, p=p)

            indicies.append(population[threadNum].index(parent1))
            indicies.append(population[threadNum].index(parent2))
        ...
        population[threadNum] = newPopulation
        assert(len(population[threadNum]) == POP_SIZE)
...
thread1 = threading.Thread(target=simulateGeneration, args=(0.980, 0,))
thread2 = threading.Thread(target=simulateGeneration, args=(0.985, 1,))
thread3 = threading.Thread(target=simulateGeneration, args=(0.990, 2,))
thread4 = threading.Thread(target=simulateGeneration, args=(0.995, 3,))
